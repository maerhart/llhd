...
llhd.proc @acc_ff(%clk : !llhd.sig<i1>, %d : !llhd.sig<i32>) -> (%q : !llhd.sig<i32>) {
    br ^init
^init:
    %clk0 = llhd.prb %clk : !llhd.sig<i1> -> i1
    llhd.wait %clk, ^check : !llhd.sig<i1>
^check:
    %clk1 = llhd.prb %clk : !llhd.sig<i1> -> i1
    %chg = cmpi "ne", %clk0, %clk1 : i1
    %posedge = llhd.and %chg, %clk1 : i1
    cond_br %posedge, ^init, ^event
^event:
    %dp = llhd.prb %d : !llhd.sig<i32> -> i32
    %delay = llhd.const #llhd.time<1ns, 0d, 0e> : !llhd.time
    llhd.drv %q, %dp, %delay : !llhd.sig<i32>, i32, !llhd.time
    br ^init
}
...
llhd.proc @acc_comb(%q : !llhd.sig<i32>, %x : !llhd.sig<i32>, %en : !llhd.sig<i1>) -> (%d : !llhd.sig<i32>) {
    br ^entry
^entry:
    %qp = llhd.prb %q : !llhd.sig<i32> -> i32
    %enp = llhd.prb %en : !llhd.sig<i1> -> i1
    %delay = llhd.const #llhd.time<2ns, 0d, 0e> : !llhd.time
    llhd.drv %d, %qp, %delay : !llhd.sig<i32>, i32, !llhd.time
    cond_br %enp, ^final, ^enabled
^enabled:
    %xp = llhd.prb %x : !llhd.sig<i32> -> i32
    %sum = addi %qp, %xp : i32
    llhd.drv %d, %sum, %delay : !llhd.sig<i32>, i32, !llhd.time
    br ^final
^final:
    llhd.wait %q, %x, %en, ^entry : !llhd.sig<i32>, !llhd.sig<i32>, !llhd.sig<i1>
}
